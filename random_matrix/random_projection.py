    ...
    for i in range(0, len(files), args.batch_size):
        ...
        for filepath in batch_files:
            try:
                ...
                # Generate 10 evenly spaced dimensions between 2 and n_features-1
                dims = np.linspace(2, max(3, n_features - 1), 10, dtype=int)
                dims = sorted(list(set(dims)))  # Remove duplicates and sort
                dims = [int(d) for d in dims if d < n_features]  # Convert to Python int
                
                # 1. Baseline Task
                batch_tasks.append((str(filepath), 'baseline', None, n_features))
                
                # 2. Projection Tasks (10 seeds * 10 dims = 100)
                if dims:
                    seeds = [42, 123, 999, 456, 789, 111, 222, 333, 444, 555]
                    for seed in seeds:
                        for d in dims:

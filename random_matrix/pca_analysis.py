...
class PCAAnalysis:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Minimum recall threshold: {self.min_recall_threshold*100:.0f}% of baseline"
        )
        self.logger.info(
            f"F2 weight: {self.f2_weight*100:.0f}%, Time weight: {self.time_weight*100:.0f}%"
        )
    ...
    def train_and_evaluate_silent(
        self, X_train, X_val, X_test, y_train, y_val, y_test, params
    ):
        ...
        try:
            ...
            # Find optimal threshold on validation set (maximize F2)
            best_threshold = 0.5
            best_f2 = 0
            for t in np.arange(0.1, 0.9, 0.01):
                y_pred_temp = (y_val_pred >= t).astype(int)
                f2_temp = fbeta_score(
                    y_val, y_pred_temp, beta=2, zero_division=0
                )
                if f2_temp > best_f2:
                    best_f2 = f2_temp
                    best_threshold = t
            ...
            train_metrics = self.calculate_all_metrics(
                y_train, y_train_pred, threshold
            )
            val_metrics = self.calculate_all_metrics(
                y_val, y_val_pred, threshold
            )
            test_metrics = self.calculate_all_metrics(
                y_test, y_test_pred, threshold
            )
    ...
    def test_dataset_frozen(self, dataset_name, filepath):
        ...
        self.logger.info("Using frozen default hyperparameters")

        # Test with all original features first
        self.logger.info("Testing with ALL ORIGINAL FEATURES...")
        ...
        if metrics_all is None:
            self.logger.warning(
                "Failed to train with all features. Skipping this dataset."
            )
            return
        ...
        results = {
            ...
            "efficiency_score": [1.0],  # Baseline gets perfect score
            "acceptable": [True],
        }
        ...
        configs_with_variance = [
            (n, explained_vars[n]) for n in component_counts
        ]
        batch_size = max(3, len(component_counts) // (self.n_jobs * 2))
        batches = [
            configs_with_variance[i : i + batch_size]
        ...
        batch_results = Parallel(
            n_jobs=self.n_jobs, verbose=0, prefer="processes", batch_size=1
        )(
        ...
        for batch in batch_results:
            for result in batch:
                ...
                score, acceptable = self.compute_efficiency_score(
                    result["test_f2"],
                    baseline_f2,
                    result["test_recall"],
                    baseline_recall,
                ...
                score_str = (
                    f"{score:.4f}" if acceptable else f"❌ {score:.2f}"
                )
                self.logger.debug(
                    f"[{dataset_name}] Frozen | {result['n_components']:3d} comp | "
                    f"F2={result['test_f2']:.4f} | Recall={result['test_recall']:.4f} | "
                    f"Time={result['training_time']:.2f}s | Score={score_str}"
                )
        ...
        # Summary
        best_pca_f2 = (
            max(results["test_f2"][1:]) if len(results["test_f2"]) > 1 else 0
        )
        best_idx = (
            results["test_f2"].index(best_pca_f2) if best_pca_f2 > 0 else 0
        )
        best_score = (
            results["efficiency_score"][best_idx]
            if best_idx < len(results["efficiency_score"])
            else 0
        )

        drop = (
            (baseline_f1 - results["test_f1"][best_idx]) / baseline_f1 * 100
            if baseline_f1 > 0
            else 0
        )
    ...
    def optuna_objective_for_pca(
        self, trial, X_train, X_val, X_test, y_train, y_val, y_test
    ):
        """Optuna objective function for hyperparameter tuning (optimizes F2 score)."""
        params = {
        ...
        try:
            ...
            # Optimize F2 score (not F1)
            f2_score_val = metrics["test"]["f2"]
            trial.report(f2_score_val, step=0)
    ...
    def tune_hyperparameters_for_pca(
        self,
        X_train,
        X_val,
        X_test,
        ...
        (
            ...
            _,
            _,
        ) = self.apply_pca(X_train, X_val, X_test, n_components)
    ...
    def test_dataset_tuned(self, dataset_name, filepath):
        ...
        self.logger.info("=" * 80)
        self.logger.info(
            f"Running Optuna optimization ({self.n_optuna_trials} trials per config)"
        )
        ...
        # Test with all original features first
        self.logger.info(
            "Tuning hyperparameters for ALL ORIGINAL FEATURES..."
        )
        ...
        for n_comp in component_counts:
            ...
            self.logger.debug(
                ...
                f"F2={metrics['test']['f2']:.4f} | Recall={metrics['test']['recall']:.4f} | "
                f"Time={metrics['training_time']:.2f}s | Score={score_str}"
            )
            ...
            results["efficiency_score"].append(score)
        ...
        # Summary
        best_pca_f2 = (
            max(results["test_f2"][1:]) if len(results["test_f2"]) > 1 else baseline_f2
        )
        best_idx = (
            results["test_f2"].index(best_pca_f2) if best_pca_f2 > 0 else 0
        )
    ...
    def plot_dataset_results(self, dataset_name):
        ...
        ax.set_title(
            "[Frozen Model] All Metrics vs Components", fontweight="bold"
        )
        ...
        ax.set_ylabel("Score", fontweight="bold")
        ax.set_title(
            "[Tuned Model] All Metrics vs Components", fontweight="bold"
        )
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
        ax.set_xscale("log")
        ...
        if has_frozen:
            results = self.results_frozen[dataset_name]
            n_comp, scores = sorted_pairs(results, "efficiency_score")
            _, acceptable = sorted_pairs(results, "acceptable")

            colors = ["green" if acc else "red" for acc in acceptable]
            ax.scatter(
                n_comp,
                scores,
                c=colors,
        ...
        ax.set_title(
            "[Frozen Model] Efficiency Score vs Components", fontweight="bold"
        )
        ...
        ax.set_ylabel("Efficiency Score", fontweight="bold")
        ax.set_title(
            "[Tuned Model] Efficiency Score vs Components", fontweight="bold"
        )
        ax.legend(fontsize=9)
        ax.grid(True, alpha=0.3)
        ax.set_xscale("log")
        ...
        try:
            report = {
                ...
                "frozen": self._make_serializable(
                    self.results_frozen.get(dataset_name, {})
                ),
                "tuned": self._make_serializable(
                    self.results_tuned.get(dataset_name, {})
                ),
            }
            ...
            self.logger.error(
                f"Failed to save JSON report for {dataset_name}: {e}"
            )


def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Run PCAAnalysis for XGBoost datasets."
    )
    parser.add_argument(
        "--datasets-dir",
        "-d",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.90,
        help="Minimum recall retention ratio (0-1, default: 0.90 = 90% of baseline)",
    )
    parser.add_argument(
        "--f2-weight",
        type=float,
        default=0.70,
        help="Weight for F2 score in efficiency score (0-1, default: 0.70)",
    )
    parser.add_argument(
        "--verbose",
        "-v",
...
if __name__ == "__main__":
    ...
    if args.parallel_datasets and len(dataset_paths) > 1:
        print(
            f"Running in PARALLEL DATASET mode with {len(dataset_paths)} datasets"
        )
        print("=" * 80)
        ...
        for name, path in dataset_paths:
            ...
            if args.run in ("frozen", "both"):
                try:
                    ...
                    pa.logger.error(
                        f"Error running frozen analysis for {name}: {e}"
                    )
            ...
            if args.run in ("tuned", "both"):
                try:
                    ...
                    pa.logger.error(
                        f"Error running tuned analysis for {name}: {e}"
                    )
